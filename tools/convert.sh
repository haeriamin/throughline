...
emit_rules() {
  ...
  if [ "$fmt" = "mdc" ]; then
    ...
    {
      printf '%s\n\n' "<!-- generated by tools/convert from .throughline/adapters/source/global-rules.md + profiles/codex.profile; edit the source, not this file. -->"
      printf '# Throughline %s Codex (`AGENTS.md`)\n\n' "$DASH"
      printf 'Codex reads this `AGENTS.md` as the root agent instructions for the repo; it is also the\n'
      printf 'shared `AGENTS.md` other adapters point at as the **Codex** peer. Personas are\n'
      printf '`.codex/agents/*.toml`; commands are `.codex/prompts/*.md`.\n\n'
      printf '%s\n\n' "$global"
      printf '## Codex wiring\n\n'
      printf -- '- **Personas**: `.codex/agents/*.toml` %s spawn the matching one as a separate Codex subagent so the Reviewer stays independent.\n' "$DASH"
      printf -- '- **Commands**: `.codex/prompts/*.md` %s copy into `$CODEX_HOME/prompts/`; each points at its runbook.\n' "$DASH"
      printf -- '- **Hooks**: `.codex/hooks.json` (machine-local, wired by `tools/setup-hooks` for your OS) %s blocks writes to `/standards/` + `/exemplars/` and git push/merge.\n' "$DASH"
      printf -- '- **Canonical brain**: `.throughline/extensions/dev/commands/`, `.github/instructions/`, `/standards/`.\n\n'
      printf '> Preview adapter %s see `.codex/VERIFICATION.md` before trusting hook enforcement.\n' "$DASH"
    } | apply_slash "$sep" | write_generated "$path"
...
copy_source_tree() {
  ...
  while IFS= read -r -d '' f; do
    ...
    if [ "$marker" -eq 1 ] && [[ "$f" == *.md ]] && ! head -n1 "$f" | grep -q 'generated by tools/convert'; then
      { printf '%s\n\n' "<!-- generated by tools/convert from .throughline/adapters/source/$srcSub/$rel; edit the source, not this file. -->"
        cat "$f"
      } | write_generated "$out"
    else
...
emit_tool_docs() {
  local base="$SRC/tool-docs" rel src dest
  [ -d "$base" ] || return 0
  emit_one() {
...
else emit_shared; convert_tool "$TOOL"; fi
echo "done."
