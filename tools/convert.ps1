...
function Emit-Rules($prof, $personas, $commands) {
    ...
    elseif ($fmt -eq "codex") {
        $sb = @()
        $sb += "<!-- generated by tools/convert from .throughline/adapters/source/global-rules.md; this is the shared root AGENTS.md (Codex + GitHub Copilot CLI both load it); edit the source, not this file. -->"
        $sb += ""
        $sb += '# Throughline ' + $Dash + ' root `AGENTS.md` (Codex + Copilot CLI)'
        $sb += ''
        $sb += 'Codex reads this `AGENTS.md` as the root agent instructions for the repo, and **GitHub Copilot CLI**'
        $sb += 'loads it too (together with `.github/copilot-instructions.md` + `.github/instructions/`). It is also'
        $sb += 'the shared `AGENTS.md` other adapters point at as the **Codex** peer. Codex personas are'
        $sb += '`.codex/agents/*.toml` and commands `.codex/prompts/*.md`; Copilot CLI uses the `.github/` surface'
        $sb += 'instead (see the Copilot CLI bullet below) ' + $Dash + ' do not follow the `.codex/` paths on Copilot CLI.'
        $sb += ''
        $sb += $global
        $sb += ''
        $sb += '## Tool wiring'
        $sb += ''
        $sb += '- **Codex** ' + $Dash + ' personas `.codex/agents/*.toml` (spawn each as a separate Codex subagent so the Reviewer stays independent); commands `.codex/prompts/*.md` (copy into `$CODEX_HOME/prompts/`); hooks `.codex/hooks.json` (wired by `tools/setup-hooks`).'
        $sb += '- **GitHub Copilot CLI** ' + $Dash + ' personas and commands are the custom agents under `.github/agents/` (the eight personas + the `dev.*`/`throughline.*` command agents; delegate review to a separate agent so the Reviewer stays independent); instructions `.github/copilot-instructions.md` + `.github/instructions/`; guards `.github/hooks/copilot-cli.json`. The `.codex/` paths above do not apply.'
        $sb += '- **Both** ' + $Dash + ' blocks writes to `/standards/` + `/exemplars/` and git push/merge; canonical brain `.throughline/extensions/dev/commands/`, `.github/instructions/`, `/standards/`.'
        $sb += ''
        $sb += '> Preview adapters ' + $Dash + ' see `.codex/VERIFICATION.md` (Codex) or `docs/runtimes/copilot-cli.md` (Copilot CLI) before trusting hook enforcement.'
        Write-Generated $path (Apply-Slash ($sb -join "`n") $sep)
    }
...
function Copy-SourceTree([string]$srcSubdir, [string]$dstSubdir, [switch]$AddMarker) {
    ...
    Get-ChildItem $srcRoot -Recurse -File | ForEach-Object {
        ...
        if ($AddMarker -and $_.Extension -eq ".md" -and -not $text.StartsWith("<!-- generated")) {
            $marker = "<!-- generated by tools/convert from .throughline/adapters/source/$srcSubdir/$rel; edit the source, not this file. -->"
            if ((($text -split "`n", 2)[0]) -eq "---") {
                # Source carries YAML frontmatter; VS Code needs --- on line 1, so append the marker at the end (matches emit_personas).
                $text = $text + "`n`n" + $marker
            } else {
...
function Emit-ToolDocs($onlyTools) {
    ...
    foreach ($rel in ($map.Keys | Sort-Object)) {
        ...
        $prefix = $rel.Split('/')[0]
        if ($prefix -ne "claude-hooks" -and $null -ne $onlyTools -and ($onlyTools -notcontains $prefix)) { continue }
...
function Emit-Shared($onlyTools) {
...
if ($Tool -eq "shared") { Emit-Shared $null }
elseif ($Tool -eq "all") {
    Emit-Shared $null
