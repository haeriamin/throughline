...
# --- Claude Code: merge a hooks block into .claude/settings.local.json (only when the Claude adapter is installed) ---
$claudeInstalled = Test-Path (Join-Path $root "CLAUDE.md")
if ($claudeInstalled) {
...
# --- Codex: rebuild .codex/hooks.json for Windows (only when the Codex adapter is installed) ---
$codexPath = Join-Path $root ".codex\hooks.json"
if (Test-Path (Join-Path $root ".codex\prompts")) {
...
if ($claudeInstalled) { Write-Host "  Claude Code: .claude/settings.local.json (machine-local, gitignored)" }
if (Test-Path $codexPath) { Write-Host "  Codex:       .codex/hooks.json" }
if (Test-Path (Join-Path $root ".github\hooks\hooks.json")) { Write-Host "  Copilot:     .github/hooks/hooks.json already has a per-OS override; no action needed." }
